...
# Auto-detect project virtual environment site-packages if running from unactivated global python
repo_root = Path(__file__).resolve().parent
venv_site_win = repo_root / ".venv" / "Lib" / "site-packages"
venv_site_posix = repo_root / ".venv" / "lib" / f"python{sys.version_info.major}.{sys.version_info.minor}" / "site-packages"

if venv_site_win.exists() and str(venv_site_win) not in sys.path:
    sys.path.insert(0, str(venv_site_win))
elif venv_site_posix.exists() and str(venv_site_posix) not in sys.path:
    sys.path.insert(0, str(venv_site_posix))
...
with tab2:
    ...
    # Data derived from CHANGELOG / Benchmark Evaluation
    metrics_data = pd.DataFrame([
        {"Metric": "Viable Route Found (%)", "Approach": "Baseline (Zero-Shot LLM)", "Value": 100.0},
        {"Metric": "Viable Route Found (%)", "Approach": "Agent Solution (Iter 3)", "Value": 100.0},
        {"Metric": "Feasibility Pass Rate (%)", "Approach": "Baseline (Zero-Shot LLM)", "Value": 0.0},
        {"Metric": "Feasibility Pass Rate (%)", "Approach": "Agent Solution (Iter 3)", "Value": 100.0},
        {"Metric": "Hallucination Rate (%)", "Approach": "Baseline (Zero-Shot LLM)", "Value": 0.0},
        {"Metric": "Hallucination Rate (%)", "Approach": "Agent Solution (Iter 3)", "Value": 0.0},
        {"Metric": "Avg Confirmation Prob (%)", "Approach": "Baseline (Zero-Shot LLM)", "Value": 68.2},
        {"Metric": "Avg Confirmation Prob (%)", "Approach": "Agent Solution (Iter 3)", "Value": 64.8},
    ])
